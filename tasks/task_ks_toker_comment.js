...
let task = {
    ...
    log() {
        ...
        let allFile = "log/log-comment-ks-" + file + ".txt";
        Log.setFile(allFile);
    },
        ...
        while (i-- > 0) {
            ...
            while (true) {
                ...
                for (let k in comments) {
                    ...
                    if (machine.get('task_ks_toker_comment_' + douyin + '_' + comments[k].nickname, 'bool')) {
                        Log.log('重复');
                        continue;
                    }
                    ...
                    try {
                        KsComment.intoUserPage(comments[k]);
                    } catch (e) {
                        Log.log('跳转用户页失败');
                        continue;
                    }
                    ...
                    if (KsVideo.intoUserVideo()) {
                        ...
                        KsUser.focus();
                        let msg = this.getMsg(1, comments[k].nickname, KsUser.getAge(), KsUser.getGender());
                        if (msg) {
                            KsUser.privateMsg(msg.msg);
                        }
                    }
                    tCommon.back();
                    tCommon.sleep(1000);
...
let kw = machine.get('task_ks_toker_comment_kw');
Log.log("keyword：" + machine.get('task_ks_toker_comment_kw'));
if (!kw) {
    System.toast('请输入关键词');
    System.exit();
}
