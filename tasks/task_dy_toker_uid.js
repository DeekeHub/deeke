...
let accounts = storage.get('task_dy_toker_uid_account');
...
while (true) {
    ...
    try {
        //开启线程  自动关闭弹窗
        if (task.run(accounts, storage.get('task_dy_toker_uid_interval', 'int'), storage.getArray('task_dy_toker_uid_op'))) {
